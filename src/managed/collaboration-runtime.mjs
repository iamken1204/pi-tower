...
export function registerCollaborationTools(pi, call) {
	pi.on("before_agent_start", (event) => ({ systemPrompt: `${event.systemPrompt}\n\nTower thread collaboration: Use thread_list, thread_delegate, thread_tasks and thread_report for work in other threads or projects. These tools reuse this Runner's Tower connection; do not ask for PI_TOWER_URL or PI_TOWER_TOKEN or use pi-task or task.mjs for thread collaboration, even if older conversation history or a skill suggests them. Select the target by project, cwd and threadId; multiple projects can share one runnerId. Ask only when candidates remain ambiguous. thread_delegate returns admission, not completion; check task status and an explicit thread_report. Generate one UUID requestId per request, reuse it for retries, and on timeout query thread_tasks with that requestId instead of delegating with a new ID.` }));
